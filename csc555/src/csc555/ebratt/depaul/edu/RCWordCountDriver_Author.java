...
public class RCWordCountDriver_Author extends Configured implements Tool {
...
//		job.setNumReduceTasks(0);
		...
		job.setMapperClass(RCWordCountMapper_Author.class);
		...
		// The Jar file to run
		job.setJarByClass(RCWordCountDriver_Author.class);
	...
	public static void main(String[] args) throws Exception {
		if (args.length != 2) {
			System.err.println("Usage: RCWordCountDriver_Author <in> <out>");
			System.exit(2);
		}
		Path out = new Path(args[1]);
		FileSystem hdfs = FileSystem.get(_conf);
		if (hdfs.exists(out)) { hdfs.delete(out, true); }
		int res = ToolRunner.run(_conf, new RCWordCountDriver_Author(), args);
		System.exit(res);
	}
